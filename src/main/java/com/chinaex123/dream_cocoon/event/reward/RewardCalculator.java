...
public class RewardCalculator {
    ...
    public static RewardData calculateReward(int currentStreak) {
        ...
        if (maxStreak > 0 && currentStreak >= maxStreak) {
            return new RewardData(DCIItems.DREAM_BAG.get(), DCIServerConfig.DREAM_BAG_AMOUNT.get());
        }
    ...
    private static RewardData calculateBoostedReward(double chance) {
        ...
        if (chance < dreamChance) {
            return new RewardData(DCIItems.DREAM_BAG.get(), DCIServerConfig.DREAM_BAG_AMOUNT.get() * 2);
        } else if (chance < dreamChance + sweetChance) {
            // 如果随机数落在甜梦包区间，返回双倍甜梦包
            return new RewardData(DCIItems.SWEET_BAG.get(), DCIServerConfig.SWEET_BAG_AMOUNT.get() * 2);
        } else {
            // 否则返回双倍好梦包
            return new RewardData(DCIItems.GOODIE_BAG.get(), DCIServerConfig.GOODIE_BAG_AMOUNT.get() * 2);
        }
    }
    ...
    private static RewardData calculateNormalReward(double chance) {
        ...
        if (chance < dreamChance) {
            ...
            return new RewardData(DCIItems.SWEET_BAG.get(), DCIServerConfig.SWEET_BAG_AMOUNT.get());
        } else {
            // 否则返回基础数量的好梦包
            return new RewardData(DCIItems.GOODIE_BAG.get(), DCIServerConfig.GOODIE_BAG_AMOUNT.get());
        }
    }
}