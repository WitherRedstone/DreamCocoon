        ...
        try {
            ...
            if (itemString.contains("[") || itemString.contains("{")) {
                try {
                    ...
                    // 使用 TagParser 解析 SNBT
                    var compoundTag = TagParser.parseCompoundFully(nbtString);
                    // 使用 Codec 解析为 ItemStack
                    stack = ItemStack.CODEC.parse(registryAccess.createSerializationContext(NbtOps.INSTANCE), compoundTag).result().orElse(ItemStack.EMPTY);
            ...
            if (stack.isEmpty()) {
                // 去除组件部分，只保留物品ID
                Identifier location = Identifier.tryParse(itemString.split("\\[")[0].split("\\{")[0]);
                if (location != null) {
                    // 从注册表获取物品对象
