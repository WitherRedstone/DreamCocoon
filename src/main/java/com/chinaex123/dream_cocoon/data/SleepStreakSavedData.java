...
public class SleepStreakSavedData extends SavedData {
    private static final Identifier DATA_ID = Identifier.tryParse("dream_cocoon:sleep_streak");

    public static final SavedDataType<SleepStreakSavedData> TYPE = new SavedDataType<>(
            DATA_ID,
            SleepStreakSavedData::new,
    ...
    public static SleepStreakSavedData load(CompoundTag tag, HolderLookup.Provider provider) {
        ...
        if (tag.contains("players")) {
            ...
            for (String key : playersTag.keySet()) {
                ...
                // 从 NBT 中读取连续睡觉天数
                playerData.consecutiveDays = playerTag.getIntOr("consecutive_days", 0);
                // 从 NBT 中读取上次睡觉的世界天数
                playerData.lastSleepDay = playerTag.getLongOr("last_sleep_day", -1L);
